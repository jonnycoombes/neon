...
namespace JCS.Neon.Glow.Statics.Cryptography
{
    ...
    public static class X509Certificate
    {
        ...
        public static X509Certificate2 ImportFromFile(string source, Func<string> pf, bool exportable = true)
        {
            ...
            Logging.Logging.Verbose(_log, $"Attempting x509 certificate load from \"{source}\"");
            if (!File.Exists(source))
            {
                Logging.Logging.Warning(_log, "Specified source for x509 certificate doesn't exist, or can't be accessed");
                throw Exception.LoggedException<X509CertificateHelperException>(_log,
